...
# idea: for each descriptor in des1, find its matching by computing the minimum L2 distance with all the descriptors in des2
...
if __name__ == '__main__':
    ...
    for i in range(len(keypoints_1)):
        pt = keypoints_1[i].pt
        plt.scatter(x=pt[0], y=pt[1], c='y', s=1)
        
    for i in range(len(keypoints_2)):
        pt = keypoints_2[i].pt
        plt.scatter(x=pt[0] + width, y=pt[1], c='y', s=1)
